  ...
  useEffect(() => {
    ...
    const fetchTKB = async () => {
      ...
      try {
        ...
        if (!token) {
          console.warn("No auth token available for advisor timetable request.");
          setLoading(false);
          return;
        }

        const res = await fetch(`${PUBLIC_API_BASE_URL}advisor/timetable`, {
          method: "GET",
          headers: {
        ...
        if (result.success) {
          const instructorName = result.data.instructor_name || "Giảng viên";

          // Trải phẳng dữ liệu schedules
          const baseEvents = result.data.courses.flatMap((course: any): Event[] => {
            const { schedules, ...courseInfo } = course;
            return schedules.map((s: any): Event => ({
              ...s,
              ...courseInfo,
        ...
        <div className={styles.monthGrid}>
          ...
          {(() => {
            ...
            for (let day = 1; day <= daysInMonth; day++) {
              ...
              cells.push(
                <div key={day} className={`${styles.monthCell} ${isToday ? styles.todayCell : ""}`}>
